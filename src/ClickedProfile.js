...
function ClickedProfile({clickedUser, isFollowing, onFollow}) {
    ...
    return (
        <div className="profileContainer">
            ...
            <div>
                {(isClicked ? <ProfilePostContainer key={clickedUser.id} posts={posts} user={clickedUser} elementRef={elementRef} /> : <div className="profilePosts">{profilePosts}</div>)}
            </div>
        </div>
    )
}
