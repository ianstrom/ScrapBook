...
function ClickedProfile({clickedUser, isFollowing, onFollow, onLike, onComment, myUser}) {
    ...
    return (
        <div className="profileContainer">
            ...
            <div>
                {(isClicked ? <ProfilePostContainer key={clickedUser.id} posts={posts} user={clickedUser} elementRef={elementRef} onLike={onLike} onComment={onComment} myUser={myUser}/> : <div className="profilePosts">{profilePosts}</div>)}
            </div>
        </div>
    )
}
